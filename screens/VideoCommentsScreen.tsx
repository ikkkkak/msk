...
import { useVideoCommentsQuery } from "../hooks/queries/useVideoCommentsQuery";
import { useCreateVideoCommentMutation } from "../hooks/mutations/useVideoMutations";
...
interface VideoCommentsScreenParams {
  videoId: number;
}

export const VideoCommentsScreen = () => {
  const { t } = useTranslation();
  const navigation = useNavigation();
  const route = useRoute();
  const { user } = useUser();
  const params = route.params as VideoCommentsScreenParams;
  ...
  const comments: VideoComment[] = commentsData ?? [];
  ...
  const handleSendComment = async () => {
    ...
    if (!videoId || !commentText.trim() || createComment.isPending) return;
