...
import { useSavedVideosQuery } from "../hooks/queries/useVideoFeedQuery";
...
export const SavedVideosScreen = () => {
  const navigation = useNavigation();
  const { t } = useTranslation();
  const { data: videos = [] } = useSavedVideosQuery();
  ...
  return (
    <>
      <View style={{ position: 'absolute', top: 50, left: 16, right: 16, zIndex: 2, alignItems: 'center' }}>
        <Text style={{ color: '#FFFFFF', fontWeight: '700', fontSize: 16 }}>{t('account.saved.videosTab', 'فيديوهات محفوظة')}</Text>
      </View>
      <FlatList
        data={videos}
