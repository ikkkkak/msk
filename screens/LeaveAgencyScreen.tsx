/**
 * LeaveAgencyScreen - Professional Screen for Leaving an Agency
 * Security-first implementation with clear warnings
 * Full screen modal with proper keyboard handling
 */
...
export const LeaveAgencyScreen: React.FC<
  RootStackScreenProps<"LeaveAgency">
> = () => {
  ...
  const organization = (route.params as any)?.organization;
          ...
          [
            {
              ...
              onPress: () => {
                setConfirmText("");
                // Navigate back to Organizations tab and refetch data
                (navigation as any).navigate("Organizations");
                // Small delay to ensure navigation completes before refetch
                setTimeout(() => {
                  queryClient.refetchQueries({ queryKey: ["user-organization"] });
                  queryClient.refetchQueries({ queryKey: ["user-properties"] });
                }, 300);
              }
            }
          ]
            ...
            <TextInput
              style={[
                ...
                confirmText.trim().toLowerCase() ===
                  requiredConfirmText.toLowerCase() && styles.confirmInputValid
              ]}
              value={confirmText}
