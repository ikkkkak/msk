...
type RouteParams = {
  ...
  onApply?: (bathrooms: number) => void;
};

const BathroomsFilterScreen = () => {
  const navigation = useNavigation();
  const route = useRoute();
  const { t } = useTranslation();
  const params = (route.params || {}) as RouteParams;

  const [selectedBathrooms, setSelectedBathrooms] = useState<number>(params.initialBathrooms || 0);

  useEffect(() => {
    if (params.initialBathrooms !== undefined) {
      setSelectedBathrooms(params.initialBathrooms);
    }
  }, [params.initialBathrooms]);

  const handleApply = () => {
    params.onApply?.(selectedBathrooms);
    navigation.goBack();
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t('filters.bathrooms', 'Bathrooms')}</Text>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Select Bathrooms</Text>
          <View style={styles.chipGrid}>
            {[0, 1, 2, 3, 4, 5, 6, 7, 8].map((count) => (
              <TouchableOpacity
                key={count}
                style={[styles.chip, selectedBathrooms === count && styles.chipActive]}
                onPress={() => setSelectedBathrooms(count)}
              >
                <Text style={[styles.chipText, selectedBathrooms === count && styles.chipTextActive]}>
...
export default BathroomsFilterScreen;
