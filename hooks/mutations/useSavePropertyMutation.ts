...
export const useSavePropertyMutation = () => {
  ...
  return useMutation({
    mutationFn: ({ propertyID, op }: { propertyID: number; op: "add" | "remove" }) =>
      saveOrUnsaveProperty(propertyID, op, user?.ID, user?.accessToken),
    onMutate: async ({ propertyID, op }) => {
      await queryClient.cancelQueries({ queryKey: queryKeys.savedProperties });
      await queryClient.cancelQueries({ queryKey: queryKeys.searchProperties });
      await queryClient.cancelQueries({ queryKey: queryKeys.selectedProperty });
      ...
      onSettled: () => {
        queryClient.invalidateQueries({ queryKey: queryKeys.savedProperties });
